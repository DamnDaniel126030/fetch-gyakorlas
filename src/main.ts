...
const url : string = "https://retoolapi.dev/eqhMBH/data";
...
document.addEventListener("DOMContentLoaded", async () =>{
  ...
  async function downloadItems() {
    try {
      ...
      console.error("There was a problem loading the database: " + error.message);
    }
  }
  ...
  //Form megnyitása, táblázat eltüntetése
  ...
  itemForm.addEventListener("submit", async e => {
    ...
    if (!newItem.rating || !newItem.status || !newItem.product_id){
      alert("All fields are required to have inputs!")
    }
    else {
  ...
  async function addItem(newItem : CreateItemDto){
    try {
      await fetch(url, {
        method: "POST",
      ...
      console.error("There was an error creating this record: " + error.message)
    }
  }
})